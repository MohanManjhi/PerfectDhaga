...
//       COALESCE(bd.price, fcd.total_price, hcd.price) AS price, dp.photo_path
//       FROM designs d
...
//     const [rows] = await db.query(query);  // Ensure you're using the correct pool query method

//     if (rows.length === 0) {
//       return res.status(404).json({ message: 'No designs found' });
//     }

//     res.status(200).json(rows);  // Return the designs as JSON
//   } catch (error) {
